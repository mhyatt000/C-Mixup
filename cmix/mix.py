...
from cmix.utils import stats_values


def get_mixup_sample_rate(cfg, data_packet, use_kde=False):
    mix_idx = []
    ...
    for i in range(N):
        if cfg.mixtype == "kde" or use_kde:  # kde
            data_i = data_list[i]
            ...
            ######### get kde sample rate ##########
            kd = KernelDensity(kernel=cfg.kde_type, bandwidth=cfg.kde_bandwidth).fit(data_i)  # should be 2D
        ...
        if cfg.show_process and i == 0:
            print(f"bw = {cfg.kde_bandwidth}")
...
def get_batch_kde_mixup_idx(cfg, batch_X, batch_Y):
    assert batch_X.shape[0] % 2 == 0
    batch_packet = {}
    batch_packet["x_train"] = batch_X.cpu()
    batch_packet["y_train"] = batch_Y.cpu()

    batch_rate = get_mixup_sample_rate(cfg, batch_packet, use_kde=True)  # batch -> kde
    if cfg.show_process:
...
def get_batch_kde_mixup_batch(cfg, batch_X1, batch_X2, batch_Y1, batch_Y2):
    batch_X = torch.cat([batch_X1, batch_X2], dim=0)
    batch_Y = torch.cat([batch_Y1, batch_Y2], dim=0)

    idx2 = get_batch_kde_mixup_idx(cfg, batch_X, batch_Y)
