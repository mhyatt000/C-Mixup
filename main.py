...
parser.add_argument("--dataset", type=str, default="NO2", help="dataset")
parser.add_argument("--mixtype", type=str, default="random", help="random or kde or erm")
parser.add_argument("--use_manifold", type=int, default=1, help="use manifold mixup or not")
parser.add_argument("--seed", type=int, default=0, help="seed")
parser.add_argument("--gpu", type=int, default=0, help="train on which cuda device")

#### kde parameter ####
parser.add_argument("--kde_bandwidth", type=float, default=1.0, help="bandwidth")
parser.add_argument("--kde_type", type=str, default="gaussian", help="gaussian or tophat")
parser.add_argument(
    "--batch_type",
    default=0,
    type=int,
    help="1 for y batch and 2 for x batch and 3 for representation",
)

#### verbose ####
parser.add_argument("--show_process", type=int, default=1, help="show rmse and r^2 in the process")
parser.add_argument("--show_setting", type=int, default=1, help="show setting")

#### model read & write ####
parser.add_argument("--read_best_model", type=int, default=0, help="read from original model")
parser.add_argument("--store_model", type=int, default=1, help="store model or not")

########## data path, for RCF_MNIST and TimeSeries #########
parser.add_argument("--data_dir", type=str, help="for RCF_MNIST and TimeSeries")

parser.add_argument("--ts_name", type=str, default="", help="ts dataset name")
...
def main():
    ...
    if args.read_best_model == 0:  # normal train
        ...
        result_dict_best = algorithm.test(
            ...
            + args.mixtype
            + ", use_manifold = "
            + str(args.use_manifold)
            + ", kde_bandwidth = "
            + str(args.kde_bandwidth)
            + ":\n",
            args.show_process,
            all_begin,
            device,
